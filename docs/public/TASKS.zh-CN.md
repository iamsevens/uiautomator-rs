...
| uiautomator-cli 基础 + 工程增强 | 1~22 已完成，23~25 未完成 | 不阻塞当前版本 |
| bugfix Selector 修复 | 1~3 全部完成 | 已满足 |
| phase2 核心增强 | 2/3/4 部分完成，1/5/6/7 未完 | 默认不阻塞当前版本 |
...
来源：`uiautomator-cli/tasks.md`

1. `Task 23` 多架构/多版本回归矩阵
状态：Open
建议级别：高
阻塞性：Post-Release（建议尽快补齐）
完成定义：
- 固化 arm64/x86_64 + 多 Android 版本回归矩阵
- 形成可重复执行脚本和结果归档规则

2. `Task 24` 安装后可用性烟测（`cargo install`）
状态：Open
建议级别：高
阻塞性：Post-Release（发布后高优先）
完成定义：
- 新环境执行 `cargo install` 后可直接 `init/status/restart/uninstall`
- 明确 PATH 与资源定位检查

3. `Task 25` Nightly 回归守门
状态：Open
建议级别：中高
阻塞性：Post-Release
完成定义：
- 定时执行回归脚本
- 消费 JSON/JUnit 结果并做失败告警

### 5.2 Phase2（核心增强，默认不阻塞当前发布）

来源：`uiautomator-phase2/tasks.md`

1. `Task 1` 细化错误类型系统
- `1.1` 已完成
- `1.2` Open：现有代码全面迁移到新错误类型
- `1.3` Open：文档/示例同步
...
1. 完成 Task 24：`cargo install` 烟测
2. 完成 Task 23：回归矩阵

理由：这两项直接影响外部用户“安装即用”的成功率。

### P1（发布后第二批）

1. 完成 Task 25：Nightly 守门
2. 推进 Phase2 Task 1.2/1.3：错误类型链路收敛

理由：提升回归可信度和错误可观测性。

### P2（稳定性增强）

1. Phase2 Task 5：API 一致性
2. Phase2 Task 6：Mock 测试体系化
3. Phase2 Task 7：缓存优化（可选）
