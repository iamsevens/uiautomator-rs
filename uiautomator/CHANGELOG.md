...
### 文档
- 统一首次使用路径与安装示例版本号，补齐中英文 README 的上手流程说明。

## [1.0.1] - 2026-03-08

### 文档
- 更新安装示例与迁移说明中的版本号，保持与当前稳定版本一致。
...
### 修复
- 修复前台应用解析在部分设备上的误判：
  - `parse_app_component_from_dump` 优先使用 `mFocusedApp/topResumedActivity/mResumedActivity`，降低输入法窗口焦点干扰。
  - 回退解析仅匹配活动/焦点相关行，避免把无关 `Window` 行误识别为前台应用。
  - 增加包名有效性过滤，避免短标识被当作包名。
- 增加对应回归测试，覆盖键盘焦点干扰与无关窗口行场景。
- 调整多设备测试在单设备环境下自动跳过，避免环境依赖导致的误失败。
