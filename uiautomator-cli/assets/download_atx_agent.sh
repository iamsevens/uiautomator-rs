...
download_or_copy_apk() {
  local file_name="$1"
  local upstream_url="$2"
  local source_path="../../uiautomator/assets/${file_name}"
  ...
  if [[ -f "$source_path" ]]; then
    echo "Copy ${file_name} from ../../uiautomator/assets"
  ...
  curl -fL "$upstream_url" -o "$file_name"
  verify_checksum "$file_name" "$(expected_apk_sha256 "$file_name")"
}
...
# Keep legacy filename for current CLI embedding behavior.
cp -f atx-agent-armv7 atx-agent
verify_checksum "atx-agent" "$(expected_atx_sha256 "armv7")"

echo "Preparing UiAutomator APK assets (version ${UIAUTOMATOR_VERSION}) ..."
download_or_copy_apk \
  "app-uiautomator.apk" \
